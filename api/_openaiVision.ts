...
const extractNormalizedTimes = (value: string) => {
...
const selectBayScreenDepartureTime = (raw: RawFlight) => {
  const sourceLineTimes = extractNormalizedTimes(asString(raw.sourceLine));
  const directStd = normalizeTime(asString(raw.std));
  const candidates = Array.from(new Set([directStd, ...sourceLineTimes].filter(Boolean)));
  ...
  const futureCandidates = candidates
    .map((candidate) => ({ candidate, timestamp: new Date(buildISODate(candidate)).getTime() }))
    .filter((item) => !Number.isNaN(item.timestamp) && item.timestamp >= now);

  if (futureCandidates.length > 0) {
    return futureCandidates.sort((a, b) => b.timestamp - a.timestamp)[0].candidate;
  }

  return candidates
    .map((candidate) => ({ candidate, timestamp: new Date(buildISODate(candidate)).getTime() }))
    .filter((item) => !Number.isNaN(item.timestamp))
    .sort((a, b) => b.timestamp - a.timestamp)[0]?.candidate || candidates[candidates.length - 1];
};
...
const normalizeFlight = (
  ...
  const hhmm = sourceType === 'bay_screen'
    ? selectBayScreenDepartureTime(raw)
    : normalizeTime(asString(raw.std));
